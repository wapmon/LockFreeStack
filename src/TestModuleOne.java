...
public class TestModuleOne {
	...
	public static class ThreadInfo extends Thread{
		...
		public void run(){
			...
			executionTimes[id] = System.currentTimeMillis() - startTime;
		}
	}
...
//		initStack();
		...
		ThreadInfo[] threads = new ThreadInfo[NUM_THREADS];
		executionTimes = new long[NUM_THREADS];
		long startTime = System.currentTimeMillis();
		for(int i = 0; i < NUM_THREADS; i++){
			currentThread = new ThreadInfo(i, ++currentData, null, null);
			currentThread.start();
		...
		System.out.println("Finished " + (NUM_THREADS * NUM_OPERATIONS) + " operations in " + ((System.currentTimeMillis()  - startTime) / 1000.0) + " seconds\n"
				+ "Average Execution time is " + (LongStream.of(executionTimes).sum() / NUM_THREADS) / 1000.0 + " seconds");
		scanner.close();
	}
